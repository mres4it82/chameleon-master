...
import '../../../../model/hr/get_em_loan_request/Data.dart';

class AdvanceRequestTable extends GetView {
  final List<EmployeeLoansRequest> selectedItems;

  AdvanceRequestTable({required this.selectedItems});
    ...
    return GetBuilder<HomeController>(builder: (controller) {
      return
        kIsWeb
            ?
        SizedBox(
          width: controller.isSmallScreen(context) ? 430
...
List<Widget> _getTitleWidget() {
  return [
    ...
    _getTitleItemWidget(LocaleKeys.advanceValue.tr, 100),
    _getTitleItemWidget(LocaleKeys.requestStatus.tr, 130),
...
List<Widget> _getTitleWidgetWeb() {
  return [
    ...
    _getTitleItemWidget(LocaleKeys.advanceValue.tr, 110),
...
List<Widget> _getTitleWidgetWebLarge() {
  return [
    ...
    _getTitleItemWidget(LocaleKeys.advanceValue.tr, 150),
    ...
    return Row(
      children: <Widget>[
        Container(
          ...
          padding: const EdgeInsets.fromLTRB(5, 0, 0, 0),
          alignment: Alignment.center,
          child: controller.allEmployeesLoansReq.value.data?[index].code ==
              null
              ? const Text('')
              : Text(
        ...
        Container(
          ...
          alignment: Alignment.center,
          child:
          controller.allEmployeesLoansReq.value.data?[index]
              .startMonths ==
              null
              ? const Text('')
              : Get
              ...
              Get
                  ...
                  .isHindi
              ? Text(
            '${intl.DateFormat('yyyy-MM').format(DateTime.parse(
                controller.allEmployeesLoansReq.value.data?[index]
                    .startMonths ?? ''))}',
          )
              : Text(
            '${intl.DateFormat('MM-yyyy').format(DateTime.parse(
                controller.allEmployeesLoansReq.value.data?[index]
                    .startMonths ?? ''))}',
          ),
        ),
        Container(
          ...
          padding: const EdgeInsets.fromLTRB(5, 0, 0, 0),
          alignment: Alignment.center,
          child: controller.allEmployeesLoansReq.value.data?[index]
              .loanValue == null
              ? const Text('')
              : Text(
              '${controller.allEmployeesLoansReq.value.data?[index]
